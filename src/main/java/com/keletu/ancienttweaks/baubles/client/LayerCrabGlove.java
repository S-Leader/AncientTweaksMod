...
public class LayerCrabGlove extends RenderLayer<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>> {
    ...
    private static boolean hasCurio(AbstractClientPlayer player, Item item) {
        return CuriosApi.getCuriosInventory(player).flatMap(handler -> handler.findFirstCurio(item)).isPresent();
    }
}